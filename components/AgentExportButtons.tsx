...
import { NETWORK_SECTIONS, AGENCY_SECTIONS, type NetworkSectionId } from '@/lib/network-export-sections'
...
export default function AgentExportButtons({ target, disabled = false, completeReport = false, scopeSummary }: { target: Target; disabled?: boolean; completeReport?: boolean; scopeSummary?: {label:string;value:string}[] }) {
  ...
  if(completeReport && (target.type==='reseau' || target.type==='agence')) return <>
    ...
    {open && <ExportDialog options={target.type==='agence'?AGENCY_SECTIONS:NETWORK_SECTIONS} busy={!!busy} error={error} disabled={disabled} scope={scopeSummary || []} close={()=>setOpen(false)} download={download}/>}
  </>
...
function ExportDialog({options,busy,error,disabled,scope,close,download}:{options:readonly (readonly [NetworkSectionId,string])[];busy:boolean;error:string;disabled:boolean;scope:{label:string;value:string}[];close:()=>void;download:(format:'pdf'|'xlsx',sections?:NetworkSectionId[])=>Promise<void>}) {
  const dialog=useRef<HTMLDialogElement>(null)
  const [format,setFormat]=useState<'pdf'|'xlsx'>('pdf'),[custom,setCustom]=useState(false),[selected,setSelected]=useState<NetworkSectionId[]>(options.map(([id])=>id))
  ...
  return <dialog ref={dialog} aria-labelledby="export-dialog-title" onCancel={e=>{e.preventDefault();if(!busy)close()}} className="m-auto max-h-[90dvh] w-[calc(100%-2rem)] max-w-2xl overflow-auto rounded-2xl bg-white p-5 text-slate-900 shadow-2xl backdrop:bg-slate-950/50">
    ...
    <fieldset disabled={busy} className="mt-5 space-y-5">
      ...
      {custom && <div className="grid gap-3 rounded-xl border p-4 sm:grid-cols-2">{options.map(([id,label])=><label key={id} className="flex items-center gap-2 text-sm"><input type="checkbox" checked={selected.includes(id)} onChange={e=>setSelected(e.target.checked?[...selected,id]:selected.filter(x=>x!==id))}/>{label}</label>)}</div>}
      <p role="status" className="text-sm text-slate-600">{format==='pdf'?'PDF':'Excel'} · {custom?`${selected.length} section(s) sélectionnée(s)`:`${options.length} sections · rapport complet`}. Les filtres et la date de génération sont toujours inclus. Les graphiques restent dans l’application.</p>
