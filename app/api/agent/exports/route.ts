...
async function exportRequest(request: Request) {
  ...
  if (agentError || !agent || !['agent', 'chef', 'responsable'].includes(agent.role) || agent.actif !== true || agent.statut !== 'actif') return fail('Compte actif autorisé requis.', 403)
  let body
  try { body = await request.json() } catch { return fail('Requête invalide.', 400) }
  if (!body || !['pdf', 'xlsx'].includes(body.format) || !['fiche', 'intervalle', 'statistiques', 'equipe', 'agence'].includes(body.type)) return fail('Format ou rapport invalide.', 400)
  if (body.type === 'equipe' && !['chef', 'responsable'].includes(agent.role)) return fail('Compte chef ou responsable requis.', 403)
  if (body.type === 'agence' && agent.role !== 'responsable') return fail('Compte responsable requis.', 403)
  // An agent always exports self; a chef may choose a target, authorized by each RPC.
  const target = ['chef', 'responsable'].includes(agent.role) && body.agentId !== undefined ? body.agentId : agent.id
  ...
  try {
    ...
    if (body.type === 'fiche') {
      ...
      if (body.type === 'agence') {
        ...
        const result = await client.rpc('rapport_agence', { p_date_debut: p.debut, p_date_fin: p.fin, p_statuts: [...new Set(body.statuts)] })
        ...
        if (agent.role === 'responsable' && (typeof body.equipeId !== 'string' || !/^[0-9a-f]{8}(-[0-9a-f]{4}){3}-[0-9a-f]{12}$/i.test(body.equipeId))) return fail('Équipe requise.', 400)
        const result = await client.rpc('rapport_equipe', { ...(agent.role === 'responsable' ? { p_equipe_id: body.equipeId } : {}), p_date_debut: p.debut, p_date_fin: p.fin, p_statuts: [...new Set(body.statuts)], p_inclure_chef: body.inclureChef })
